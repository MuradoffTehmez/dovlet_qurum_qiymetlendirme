...
def get_detailed_report_context(ishchi, dovr):
    ...
    if not cavablar.exists():
        return {
            "error": f"'{dovr.ad}' dövrü üçün {ishchi.get_full_name()} haqqında heç bir tamamlanmış qiymətləndirmə tapılmadı."
        }

    kateqoriya_neticeleri = (
        SualKateqoriyasi.objects.filter(sual__cavab__in=cavablar)
        .annotate(ortalama_xal=Avg("sual__cavab__xal"))
        .distinct()
        .order_by("ad")
    )

    yazili_reyler = (
        cavablar.exclude(metnli_rey__isnull=True)
        .exclude(metnli_rey__exact="")
        .values_list("metnli_rey", flat=True)
    )
