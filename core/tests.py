...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.shobe = Shobe.objects.create(
            ad="Test Şöbəsi", departament=self.departament
        )
...
class AuthenticationFlowsTests(BaseTestCase):
    ...
    def test_successful_registration(self):
        ...
        self.assertEqual(
            response.status_code, 302, "Qeydiyyatdan sonra yönləndirmə baş vermədi."
        )
        self.assertEqual(response.url, reverse("dashboard"))
        self.assertEqual(
            Ishchi.objects.count(),
            user_count_before + 1,
            "Yeni istifadəçi bazada yaranmadı.",
        )
    ...
    def test_login_with_email_and_username(self):
        ...
        # E-poçt ilə giriş
        self.assertTrue(
            self.client.login(username="ishchi@example.com", password="password123")
        )
        self.client.logout()
    ...
    def test_password_reset_flow(self):
        ...
        token_match = re.search(
            r"reset/[a-zA-Z0-9_-]+/([a-zA-Z0-9-_]+)/", sent_email.body
        )
        ...
        self.assertEqual(
            ...
            302,
            "Şifrə dəyişdirildikdən sonra yönləndirmə baş vermədi.",
        )
        self.assertEqual(response_post.url, reverse("password_reset_complete"))

        # Addım 5: Yeni şifrənin həqiqətən işlədiyini yoxlayırıq
        self.assertTrue(
            self.client.login(username="testishchi", password=new_password),
            "Yeni şifrə ilə daxil olmaq mümkün olmadı.",
        )
