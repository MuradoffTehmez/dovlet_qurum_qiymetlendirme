...
class IshchiUpdateForm(UserChangeForm):
    ...
    profil_sekli = forms.ImageField(
        label="Profil Şəklini Yenilə", required=False, widget=forms.FileInput
    )
    dogum_tarixi = forms.DateField(
        label="Doğum Tarixi",
        widget=forms.DateInput(attrs={"type": "date"}),
...
class IshchiPasswordChangeForm(PasswordChangeForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.add_input(
            Submit(
                "change_password", "Şifrəni Dəyişdir", css_class="btn-danger w-100 mt-3"
            )
        )
...
class HedefForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "tesvir": forms.Textarea(
                attrs={
                    "rows": 2,
                    "placeholder": "Konkret və ölçüləbilən bir hədəf yazın...",
                }
            ),
